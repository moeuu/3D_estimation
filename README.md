...
The live `live-simulation` launcher starts the dashboard server by default and
...
## Live full simulation

The physical experiment profile is owned by the shared runtime; this repository
provides a generic launcher for runtime acquisition under live spectral MLE control.
Verify the complete Geant4/model/config chain without starting a long run:
...
```bash
uv run estimate-radiation-mle live-simulation \
  --scenario .local/private-scenarios/live-scenario.json \
  --output-dir results/live-mle \
  --json
```

The private scenario is created explicitly for each run; it is not searched for or
generated by this repository. Scene variants, source cardinality, environment size,
candidate isotopes, and acquisition limits remain runtime-owned.
...
No measurement route or station list is fixed in advance. The runtime profile supplies
the station/view limits and per-view live time through the truth-free handshake rather
than an MLE config or CLI override. Source truth is never passed
...
The MLE stop decision is compound: convergence/KKT, independent 3-D pose and height
...
low expected information gain must all pass. The runtime maximum is the hard safety
bound.

See [live full-simulation launcher](docs/mle/live_full_simulation.md) for ownership,
